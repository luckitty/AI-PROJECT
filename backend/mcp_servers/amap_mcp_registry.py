...
AMAP_MCP_SELECTOR_ROUTING = """
            路由原则（仅在高德 MCP 工具中选一）：
            - amap_mcp_weather：查询城市天气预报（含多日预报、未来几天气温趋势）；city 为城市名或 adcode。
            ...
            - amap_mcp_transit：公交/地铁/火车等综合公共交通路径规划；origin、destination（经度,纬度）、city、cityd 必填；跨城须分别填起终点城市。
            - amap_mcp_personal_map：将行程导入高德生成专属地图唤端链接；org_name、line_list_json 必填；链接结果原样返回勿总结。
            - amap_mcp_invoke：其它高德 MCP 能力（逆地理、距离等）且参数明确时用；tool_name、arguments_json 必填。
