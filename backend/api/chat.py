...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        # 新请求开始前清理 stop 标记，避免上一轮中断影响本轮请求。
        if session_id:
    ...
    async def generate():
        ...
        try:
            ...
            for chunk in stream_iterator:
                ...
                # 这里是“按增量点位”检查：在拿到下一个 chunk 时会立即退出。
                if await http_request.is_disconnected():
                    # 关键：断连时同步打 stop 标记，让图内节点（尤其 response_node）感知并尽快停。
                    if session_id:
                ...
                if token:
                    ...
                    # 主动让出事件循环，降低多个小增量在服务端/网关层被合并后再下发的概率。
                    await asyncio.sleep(0)
                # 输出后再次检查断连，减少“断开后还多发几包”的窗口。
                if await http_request.is_disconnected():
                    # 二次检查同样写 stop，覆盖“发送后立刻断连”的窗口期。
                    if session_id:
                        interrupt_manager.stop(session_id)
                    break
