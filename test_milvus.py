...
# =========================
# 环境变量（可选）：
#   MILVUS_HOST / MILVUS_PORT — 默认 127.0.0.1:19530（避免 localhost→IPv6 与 Docker 端口映射不一致）
#   MILVUS_TIMEOUT — RPC 超时（秒），默认 60
#   MILVUS_COLLECTION — collection 名，默认 rag_collection
#   MILVUS_SKIP_INGEST=1 — 不写入，只连接已有库（调试检索时用）
#   MILVUS_DROP_OLD=1 — 写入前删除同名 collection（全量重建）
#
